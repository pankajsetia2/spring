...
/**
 * An AccountManager that uses Hibernate to find and update accounts.
 * Annotated as a @Repository so Spring may auto-configure an instance of this class as a bean and weave in Hibernate->DataAccessException translation behavior.
 * Methods are annotated as @Transactional so they run within a database transaction.
 */
...
public class HibernateAccountManager implements AccountManager {
	...
	public Account findAccount(String number) {
		return (Account) getCurrentSession().
			createQuery("from Account a left join fetch a.beneficiaries where a.number = ?").
	...
	public List<Account> findAccounts(AccountSearchCriteria searchCriteria) {
		...
		return getCurrentSession().createQuery("from Account a where upper(a.name) like :name order by a.name")
			.setString("name", searchString.toString())
			.setFirstResult(searchCriteria.getPage() * searchCriteria.getMaximumResults())
			.setMaxResults(searchCriteria.getMaximumResults())
			.list();
	}
